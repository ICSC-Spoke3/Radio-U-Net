...
if USE_MPI == 1:
   ...
   comm.Bcast(testX, root=0)
   comm.Bcast(trainXNoisy, root=0)
   comm.Bcast(testXNoisy, root=0)
     ...
     for imodel in hyper_dict['hyperparam']:
         #with tf.device(mydevice):
		  ...
		  # Saving the model
              ...
              if checkpoint == 1:
                 restartfile = 'model-'+str(deep_model)+'-'+str(nbatch)+'-'+str(num_epoch)+'-'+str(eta_input)+'.ckpt'
                 autoencoder.save(restartfile)
              #loss, acc = autoencoder.evaluate(testXNoisy, testX)
              #print("")
              #print("")
              ##print('MODEL ACCURACY = {:13.7f}'.format(acc))
              #print(str(deep_model)+'MODEL ACCURACY = '+str(acc))
              #print("")
              #print("")
...
if plotdata == 1:
  ...
  for tiletoshow in range(nouts):
   ...
   plt.title("Mask map")
   plt.subplot(223)
   plt.imshow(skydata[tiletoshow,:,:],cmap='magma')
   #plt.colorbar(label=r"$\rm S_{x, \ 0.3-2.0 \ keV} \ [cnt]$", orientation="horizontal", fraction=0.04)
   plt.title("Sky map")
   plt.subplot(224)
